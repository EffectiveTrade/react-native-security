...
@synthesize willSecItemAdd = _willSecItemAdd;
...
#endif
        if(self.willSecItemAdd) {
            self.willSecItemAdd(query);
        }
...
// DT_MARK
- (BOOL)deleteTheItem:(NSError *__autoreleasing *)error {
    OSStatus status = SAMKeychainErrorBadArguments;
    if (!self.service) {
        if (error) {
            *error = [[self class] errorWithCode:status];
...
// DT_MARK
- (BOOL)theFetch:(NSError *__autoreleasing *)error {
    OSStatus status = SAMKeychainErrorBadArguments;
    if (!self.service) {
        if (error) {
            *error = [[self class] errorWithCode:status];
