...
+ (nullable NSString *)passwordForService:(NSString *)serviceName {
    return [self passwordForService:serviceName error:nil];
}

+ (nullable NSString *)passwordForService:(NSString *)serviceName error:(NSError *__autoreleasing *)error {
    SAMKeychainQuery *query = [[SAMKeychainQuery alloc] init];
    query.service = serviceName;
    [query theFetch:error];
    return query.password;
}
...
+ (BOOL)deletePasswordForService:(NSString *)serviceName {
    return [self deletePasswordForService:serviceName error:nil];
}

+ (BOOL)deletePasswordForService:(NSString *)serviceName error:(NSError *__autoreleasing *)error {
    SAMKeychainQuery *query = [[SAMKeychainQuery alloc] init];
    query.service = serviceName;
    return [query deleteTheItem:error];
}
